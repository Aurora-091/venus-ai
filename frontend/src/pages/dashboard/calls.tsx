...
export default function Calls({ tenant }: Props) {
  ...
  const handleOutbound = async () => {
    ...
    setCallMsg('');
    try {
      const result = await api.post(`/tenants/${tenant.id}/outbound-call`, {
  ...
  const filteredLocal =
    filter === 'all'
      ? localCalls
      : localCalls.filter((c) => c.outcome === filter);

  const outcomeColor = (o: string) =>
    o === 'completed' || o === 'success'
      ? 'bg-emerald-500/10 text-emerald-400'
      : o === 'escalated' || o === 'failure'
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-[#475569] mt-0.5">
            {totalCalls}{' '}
            {tab === 'live' ? 'ElevenLabs conversations' : 'local records'}
          </p>
        </div>
      ...
      {callMsg && (
        <div
          className={`mb-4 text-xs px-4 py-2.5 rounded-xl ${callMsg.startsWith('Failed:') ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}
        >
          {callMsg}
        </div>
      )}
        ...
        <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl overflow-hidden">
          <div className="grid grid-cols-[1fr_90px_80px_100px_120px_80px] px-5 py-3 border-b border-[#1E2A3E]">
            {[
              ...
              <div key={h} className="text-xs font-medium text-[#475569]">
                {h}
              </div>
            ))}
          </div>
          {loading ? (
            <div className="px-5 py-10 text-center text-sm text-[#475569]">
              Loading...
            </div>
          ) : liveCalls.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <div className="text-sm text-[#475569] mb-2">
                No ElevenLabs conversations yet
              </div>
              <div className="text-xs text-[#2A3A54]">
                {hasRealAgent
                  ? `Make or receive a call via ${tenant?.phoneNumber || 'your Twilio number'} to see live transcripts here`
                  : 'Connect a real ElevenLabs agent to enable live transcripts.'}
              </div>
            </div>
          ) : (
                ...
                >
                  <div>
                    <div className="text-sm font-medium text-white flex items-center gap-2">
                      ...
                      <span className="bg-blue-500/10 text-blue-400 text-[9px] px-1.5 py-0.5 rounded border border-blue-500/20 uppercase tracking-wider">
                        Inquiry
                      </span>
                      <span className="bg-emerald-500/10 text-emerald-400 text-[9px] px-1.5 py-0.5 rounded border border-emerald-500/20 uppercase tracking-wider">
                        Neutral
                      </span>
                    </div>
                  ...
                  <div className="flex items-center">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${conv.direction === 'outbound' ? 'bg-blue-500/10 text-blue-400' : 'bg-[#1E2A3E] text-[#94A3B8]'}`}
                    >
                  ...
                  <div className="flex items-center">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${outcomeColor(conv.call_successful || '')}`}
                    >
      ...
      {tab === 'local' && (
        <>
          <div className="flex gap-2 mb-4">
            {(['all', 'completed', 'escalated', 'missed'] as Outcome[]).map(
              (f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  ...
                  >
                    <div>
                      <div className="text-sm font-mono flex items-center gap-2">
                        ...
                        <span className="bg-purple-500/10 text-purple-400 text-[9px] px-1.5 py-0.5 rounded border border-purple-500/20 uppercase tracking-wider">
                          Support
                        </span>
                        <span className="bg-emerald-500/10 text-emerald-400 text-[9px] px-1.5 py-0.5 rounded border border-emerald-500/20 uppercase tracking-wider">
                          Positive
                      ...
                      <div className="text-xs text-[#475569] mt-0.5 truncate max-w-xs">
                        {call.summary || '—'}
                      </div>
                    </div>
                    <div className="flex items-center">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${call.direction === 'outbound' ? 'bg-blue-500/10 text-blue-400' : 'bg-[#1E2A3E] text-[#94A3B8]'}`}
                      >
                        {call.direction || 'in'}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${outcomeColor(call.outcome)}`}
                      >
                        {call.outcome || 'unknown'}
                      </span>
                    </div>
                    <div className="flex items-center text-xs text-[#475569]">
                      {call.createdAt?.slice(0, 10) || '—'}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#1E2A3E] flex items-center justify-between">
            ...
            <div className="text-xs text-[#475569]">
              Preview generated tickets and actions taken by agent
            </div>
          </div>
          <div className="divide-y divide-[#1E2A3E]">
            {[
              {
                id: 'TKT-001',
                title: 'Refund requested for missing item',
                type: 'Ticket',
                status: 'Needs Review',
                caller: '+14155552671',
                date: '2 hrs ago',
              ...
              {
                id: 'ACT-045',
                title: 'Sent follow-up SMS with booking link',
                type: 'Action',
                status: 'Completed',
                caller: '+12125550982',
                date: '5 hrs ago',
              ...
              {
                id: 'TKT-002',
                title: 'Escalated: Customer extremely upset',
                type: 'Ticket',
                status: 'Open',
                caller: '+15125558833',
                date: '1 day ago',
              ...
              <div
                key={i}
                className="px-5 py-4 hover:bg-[#0A0E1A] transition-colors cursor-pointer flex items-center justify-between"
              >
                <div>
        ...
        >
          ...
          >
            ...
            <div className="overflow-y-auto p-6 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  [
                    'Duration',
                    fmtDuration(
                      selected.call_duration_secs || selected.durationSeconds
                    ),
                  ],
                  ['Direction', selected.direction || 'inbound'],
                  [
                    'Status',
                    selected.call_successful || selected.outcome || '—',
                  ],
                  [
                    'Date',
                    fmtDate(
                      selected.start_time_unix_secs || selected.createdAt
                    ),
                  ],
                  [
                    'Messages',
                    selected.message_count
                      ? `${selected.message_count} turns`
                      : '—',
                  ],
                  ...
                  <div
                    key={label}
                    className="bg-[#080C14] rounded-xl px-3 py-2.5"
                  >
              ...
              {/* Audio player */}
              {(selected.has_audio || convDetail?.has_audio) &&
                selected.conversation_id && (
                  <div>
                    <div className="text-xs font-medium text-[#94A3B8] mb-2 uppercase tracking-wide">
                      Call Recording
                    </div>
                    <audio
                      controls
              ...
              <div>
                <div className="text-xs font-medium text-[#94A3B8] mb-3 uppercase tracking-wide">
                  Transcript{' '}
                  {loadingDetail && (
                    <span className="text-[#475569] normal-case ml-2">
                      Loading...
                    </span>
                  )}
                </div>
                {loadingDetail ? (
                  <div className="space-y-2">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-10 bg-[#1E2A3E] rounded-xl animate-pulse"
                      />
                    ))}
                  </div>
                  ...
                  <div className="bg-[#080C14] rounded-xl p-4 text-sm text-[#94A3B8]">
                    {selected.transcript}
                  </div>
                ) : !selected.conversation_id ? (
                  <div className="text-xs text-[#475569]">
                    Transcript not available for local-only records.
                  </div>
                ) : (
                  <div className="text-xs text-[#475569]">
                  ...
                  <div className="space-y-1.5">
                    {convDetail.transcript
                      ...
                      .map((tr: any, i: number) => (
                        <div
                          key={i}
                          className="bg-[#080C14] border border-[#1E2A3E] rounded-xl px-4 py-2.5 flex items-center gap-3"
                        >
                          <span className="text-xs font-mono text-emerald-400">
                            {tr.tool_name}
                          </span>
                          <span className="text-xs text-[#475569]">
                            {JSON.stringify(tr.params_as_json)}
                          </span>
                        </div>
                      ))}
                  </div>
