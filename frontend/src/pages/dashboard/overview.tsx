      ...
      {chartData.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <div
            ...
            style={{
              height: `${Math.round((d.count / max) * 52)}px`,
...
export default function Overview({ tenant }: Props) {
    ...
    return (
      <div className="p-8">
        <div className="max-w-md mx-auto text-center mt-20">
          <div className="w-16 h-16 rounded-2xl bg-[#0F1623] border border-[#1E2A3E] flex items-center justify-center text-3xl mx-auto mb-4">
            ◈
          </div>
          ...
          <Link
            href="/onboarding"
            className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-colors"
          >
            Set up workspace →
          </Link>
        </div>
  ...
  const verticalColor =
    {
      clinic: 'text-emerald-400',
      hotel: 'text-blue-400',
      ecommerce: 'text-violet-400',
    }[tenant.vertical || ''] || 'text-blue-400';
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-xs text-[#94A3B8]">
                Connect ElevenLabs to activate your voice agent
              </div>
            </div>
          </div>
          <Link
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="bg-[#111] border border-[#222] rounded-xl p-5 animate-pulse h-24"
            />
          ))}
        </div>
      ) : analytics ? (
        ...
        {tenant.vertical === 'ecommerce' ? (
          <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl p-5 flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs text-[#475569] mb-4">
                Most common order issues detected
              </p>
            ...
            <Link
              href="/dashboard/agent"
              className="text-xs text-blue-400 hover:text-blue-300 transition-colors mt-4 block"
            >
              Train Agent →
            </Link>
          </div>
          ...
          <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl p-5 flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs text-[#475569] mb-4">
                Caller intents detected this week
              </p>
              <div className="space-y-3">
                {[
      ...
      <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl">
        ...
        {calls.length === 0 ? (
          <div className="px-5 py-10 text-center text-sm text-[#475569]">
            No calls yet. Your agent is ready to answer.
          </div>
        ) : (
            ...
            {calls.map((call, i) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-2 h-2 rounded-full ${call.outcome === 'completed' ? 'bg-emerald-400' : call.outcome === 'escalated' ? 'bg-amber-400' : 'bg-[#475569]'}`}
                  />
                  <div>
                    <div className="text-sm font-mono">
                      {call.callerNumber || 'Unknown'}
                    </div>
                    <div className="text-xs text-[#475569] mt-0.5 max-w-xs truncate">
                      {call.summary || '—'}
                    </div>
                  </div>
                </div>
