...
export default function Integrations({ tenant }: Props) {
  const [calStatus, setCalStatus] = useState<{ connected: boolean; calendarId?: string } | null>(null);
  const [sheetsStatus, setSheetsStatus] = useState<{ connected: boolean; spreadsheetId?: string; spreadsheetUrl?: string; lastSync?: string } | null>(null);
  ...
  const loadStatus = async () => {
    ...
    try {
      const [cal, sheets] = await Promise.all([
        api.get(`/tenants/${tenant.id}/calendar/status`).catch(() => ({ connected: false })),
        api.get(`/tenants/${tenant.id}/sheets/status`).catch(() => ({ connected: false })),
      ]);
      setCalStatus(cal);
  ...
  const handleCalConnect = async () => {
    ...
    try {
      ...
      if (!r.url) { showMsg("Could not get auth URL", false); return; }
      const popup = window.open(r.url, "gcal_auth", "width=600,height=700,left=300,top=100");
    ...
    } catch (e: any) { showMsg("Failed: " + e.message, false); }
    setConnecting(null);
  };
  ...
  const handleSheetsCreate = async () => {
    if (!tenant?.id) return;
    if (!calStatus?.connected) { showMsg("Connect Google Calendar first — it shares the same login.", false); return; }
    setConnecting("sheets");
    try {
      const r = await api.post(`/tenants/${tenant.id}/sheets/create`, {});
      if (!r.success) { showMsg(r.error || "Failed to create sheet", false); return; }
      await loadStatus();
  ...
  const handleSheetsLink = async () => {
    ...
    try {
      ...
      const r = await api.post(`/tenants/${tenant.id}/sheets/link`, { spreadsheetId: parsed });
      if (!r.success) { showMsg(r.error || "Could not link sheet", false); return; }
      await loadStatus();
  ...
  const handleSheetsSync = async () => {
    ...
    try {
      ...
      if (!r.success) { showMsg(r.error || "Sync failed", false); return; }
      await loadStatus();
      showMsg(`Synced ${r.rowsSynced} rows to Google Sheets`);
    } catch (e: any) { showMsg("Sync failed: " + e.message, false); }
    setSyncing(false);
  };
  ...
  const handleExcelExport = async () => {
    ...
    setExporting(true);
    try {
      const [callsRes, bookingsRes, ordersRes, analyticsRes] = await Promise.all([
      ...
      const wsC = XLSX.utils.json_to_sheet(callRows.length ? callRows : [{ Date: "", "Caller Number": "", Direction: "", "Duration (s)": "", Outcome: "", Summary: "" }]);
      ...
      const wsB = XLSX.utils.json_to_sheet(bookingRows.length ? bookingRows : [{}]);
    ...
    } catch (e: any) { showMsg("Export failed: " + e.message, false); }
    setExporting(false);
  };
  ...
  if (!tenant) return <div className="p-8 text-sm text-[#475569]">Select a workspace first.</div>;

  return (
    <div className="p-6 max-w-3xl space-y-5">
      <div className="mb-2">
        <h1 className="text-xl font-semibold">Integrations</h1>
        <p className="text-sm text-[#475569] mt-0.5">Connect tools to extend your agent's capabilities</p>
      </div>

      {msg && (
        <div className={`text-xs px-4 py-3 rounded-xl ${msg.ok ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
          {msg.text}
        </div>
      )}
      ...
      <IntCard
        ...
        badge={calStatus?.connected ? calStatus.calendarId || "primary" : undefined}
        onConnect={handleCalConnect}
        onDisconnect={handleCalDisconnect}
        connectLabel="Connect Calendar"
        connectedDetail={calStatus?.connected ? "Calendar connected — agent uses real availability" : undefined}
      />
      ...
      <div className={`bg-[#0F1623] border rounded-2xl p-5 transition-colors ${sheetsStatus?.connected ? "border-emerald-500/30" : "border-[#1E2A3E]"}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-[#1E2A3E] text-[#475569]">CRM</span>
              ...
              <p className="text-xs text-[#475569] mt-0.5">Auto-sync all call logs, bookings, orders, and analytics to a live Google Sheet. Syncs after every booking.</p>
            </div>
          </div>
          ...
          {!sheetsStatus?.connected ? (
            <div className="flex items-center gap-2 shrink-0">
              {!calStatus?.connected && (
                <span className="text-xs text-amber-400">Connect Calendar first</span>
              )}
              <button
        ...
        {showLinkInput && !sheetsStatus?.connected && (
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={() => setShowLinkInput(false)} className="text-[#475569] text-xs px-3 py-2">Cancel</button>
          </div>
        )}
        ...
        {sheetsStatus?.connected && (
          <div className="mt-4 grid grid-cols-3 gap-3">
            <InfoTile label="Sheet ID" value={sheetsStatus.spreadsheetId?.slice(0, 24) + "…" || "—"} mono />
            <InfoTile label="Last Synced" value={sheetsStatus.lastSync ? new Date(sheetsStatus.lastSync).toLocaleString("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "Never"} />
        ...
        {sheetsStatus?.connected && (
          <div className="mt-4 border-t border-[#1E2A3E] pt-4 grid grid-cols-4 gap-2">
            {[
              { tab: "Call Logs", cols: "Date, Caller, Duration, Outcome, Summary" },
              { tab: "Bookings", cols: "Customer, Phone, Service, Slot, Status" },
              { tab: "Orders", cols: "Order #, Customer, Items, Total, Status" },
              { tab: "Analytics", cols: "Total calls, Resolution rate, Avg duration" },
            ].map(({ tab, cols }) => (
              <div key={tab} className="bg-[#080C14] rounded-xl p-3">
                <div className="text-xs font-medium text-emerald-400 mb-1">{tab}</div>
                <div className="text-xs text-[#475569] leading-relaxed">{cols}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-[#1E2A3E] text-[#475569]">Export</span>
              </div>
              <p className="text-xs text-[#475569] mt-0.5">Download a .xlsx file with all data — Call Logs, Bookings, Orders, Analytics. Opens directly in Excel or Google Sheets.</p>
            </div>
          </div>
          <button
        ...
        <div className="mt-4 grid grid-cols-4 gap-2">
          {["Call Logs", "Bookings", "Orders", "Analytics"].map(t => (
            <div key={t} className="bg-[#080C14] border border-[#1E2A3E] rounded-lg px-3 py-2 text-xs text-[#475569] text-center">{t}</div>
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-[#1E2A3E] pt-5">
        <div className="text-xs font-medium text-[#475569] uppercase tracking-wide mb-3">Core (Auto-configured)</div>
        <div className="grid grid-cols-2 gap-3">
          <CoreCard icon="🎙" title="ElevenLabs ConvAI" desc="Voice engine + STT + TTS + agent orchestration" status="Live" />
          <CoreCard icon="📞" title="Twilio" desc={`Phone ${tenant.phoneNumber || "+17407465110"} — inbound + outbound`} status="Live" />
        </div>
      </div>

      {/* ── Coming soon ── */}
      <div className="border-t border-[#1E2A3E] pt-5">
        <div className="text-xs font-medium text-[#475569] uppercase tracking-wide mb-3">Coming Soon</div>
        <div className="grid grid-cols-2 gap-3">
          <CoreCard icon="💬" title="Slack Alerts" desc="Notify your team when calls escalate" status="Soon" />
          <CoreCard icon="📱" title="WhatsApp Business" desc="Same agent on WhatsApp via ElevenLabs" status="Soon" />
          <CoreCard icon="🔁" title="n8n / Zapier" desc="Connect any app via webhook automation" status="Soon" />
          <CoreCard icon="☁️" title="HubSpot CRM" desc="Auto-push contacts and call summaries" status="Soon" />
        </div>
      </div>
    </div>
  );
...
function IntCard({ icon, title, category, desc, connected, loading, badge, onConnect, onDisconnect, connectLabel, connectedDetail }: any) {
  return (
    <div className={`bg-[#0F1623] border rounded-2xl p-5 transition-colors ${connected ? "border-blue-500/30" : "border-[#1E2A3E]"}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl border flex items-center justify-center text-xl shrink-0 ${connected ? "bg-blue-500/10 border-blue-500/20" : "bg-[#1E2A3E] border-[#2A3A54]"}`}>
            {icon}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-medium text-sm">{title}</span>
              <span className="text-xs px-2 py-0.5 rounded-full bg-[#1E2A3E] text-[#475569]">{category}</span>
              {connected && <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">Connected</span>}
            </div>
        ...
        <div className="shrink-0">
          {connected ? (
            <button onClick={onDisconnect} className="text-[#475569] hover:text-red-400 text-xs px-3 py-2 rounded-xl transition-colors border border-[#1E2A3E] hover:border-red-500/20">
              Disconnect
            </button>
          ) : (
      ...
      {connectedDetail && (
        <div className="mt-3 text-xs text-[#475569] bg-[#080C14] rounded-lg px-3 py-2">{connectedDetail}</div>
      )}
    </div>
  );
}
    ...
    <div className="bg-[#080C14] border border-[#1E2A3E] rounded-xl p-4 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full ${status === "Live" ? "bg-emerald-500/10 text-emerald-400" : "bg-[#1E2A3E] text-[#475569]"}`}>
            {status}
          </span>
        </div>
...
function InfoTile({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="bg-[#080C14] rounded-xl px-3 py-2.5">
      ...
      <div className={`text-xs font-medium text-white truncate ${mono ? "font-mono" : ""}`}>{value}</div>
    </div>
  );
}
...
function CalIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      <rect x="3" y="4" width="18" height="18" rx="2" stroke="#3b82f6" strokeWidth="1.5"/>
      ...
      <rect x="11" y="14" width="3" height="3" rx="0.5" fill="#3b82f6" opacity="0.5"/>
    </svg>
  );
}
...
function SheetsIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M3 9h18M3 15h18M9 3v18M15 3v18" stroke="#22c55e" strokeWidth="1" opacity="0.5"/>
