...
const TIMEZONES = [
  "Asia/Kolkata", "Asia/Dhaka", "Asia/Karachi", "Asia/Dubai",
  "Asia/Singapore", "Asia/Tokyo", "Europe/London", "Europe/Berlin",
  "America/New_York", "America/Los_Angeles", "America/Chicago",
];

const PLANS = [
  { id: "starter", name: "Starter", price: "₹4,999/mo", calls: "500 calls", features: ["1 agent", "Basic analytics", "Email support"] },
  { id: "growth", name: "Growth", price: "₹14,999/mo", calls: "2,000 calls", features: ["3 agents", "Advanced analytics", "Google Calendar", "Priority support"] },
  { id: "enterprise", name: "Enterprise", price: "Custom", calls: "Unlimited", features: ["Unlimited agents", "Custom integrations", "Dedicated support", "SLA"] },
];
...
export default function Settings({ tenant, onTenantUpdate }: Props) {
  ...
  useEffect(() => {
    if (!tenant) return;
    setForm({ name: tenant.name || "", timezone: tenant.timezone || "Asia/Kolkata" });
  }, [tenant?.id]);
  ...
  const handleDeleteWorkspace = async () => {
    if (!tenant?.id) return;
    const confirmed = window.confirm(`Delete workspace "${tenant.name}"? This cannot be undone.`);
    if (!confirmed) return;
    try {
  ...
  if (!tenant) {
    return <div className="p-8 text-sm text-[#475569]">Select a workspace.</div>;
  }
    ...
    <div className="p-6 max-w-2xl">
      ...
      {tab === "general" && (
        <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl p-6 space-y-5">
          <div>
            <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Workspace name</label>
            <input
              value={form.name}
          ...
          <div>
            <label className="block text-xs font-medium text-[#94A3B8] mb-1.5">Timezone</label>
            <select
              value={form.timezone}
          ...
          <div className="pt-1">
            ...
            <div className="font-mono text-xs text-[#475569] bg-[#080C14] px-3 py-2 rounded-lg">{tenant.id}</div>
          </div>
          ...
          {PLANS.map(plan => (
            <div
              ...
              className={`bg-[#0F1623] border rounded-2xl p-5 transition-colors ${
                plan.id === tenant.plan ? "border-blue-500/40" : "border-[#1E2A3E]"
              }`}
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {plan.id === tenant.plan && (
                      <span className="text-[10px] bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-full">Current</span>
                    )}
                  </div>
                  <div className="text-xs text-[#475569] mt-0.5">{plan.calls}</div>
                </div>
                <div className="text-right">
                  <div className="font-mono text-sm">{plan.price}</div>
                  {plan.id !== tenant.plan && plan.id !== "enterprise" && (
                    <button className="text-xs text-blue-400 hover:text-blue-300 mt-1 transition-colors">Upgrade →</button>
                  )}
                  {plan.id === "enterprise" && (
                    <button className="text-xs text-blue-400 hover:text-blue-300 mt-1 transition-colors">Contact us →</button>
                  )}
                </div>
              </div>
      ...
      {tab === "danger" && (
        <div className="space-y-4">
          <div className="bg-[#0F1623] border border-[#1E2A3E] rounded-2xl p-5">
            ...
            <div className="text-xs text-[#475569] mb-4">Sign out of your VoiceOS account on this device.</div>
            <button
          ...
          <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5">
            <div className="text-sm font-medium text-red-400 mb-1">Delete workspace</div>
            <div className="text-xs text-[#475569] mb-4">
              Permanently delete <strong className="text-white">{tenant.name}</strong> and all associated call logs, bookings, and agent configuration. This cannot be undone.
            </div>
            <button
