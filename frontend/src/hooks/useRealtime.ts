...
import { activeTenantState, realtimeEventsState, socketConnectedState, tenantsState } from "../state/appState";
...
export function useRealtime() {
  ...
  useEffect(() => {
    ...
    const pushEvent = (type: string, payload: unknown) => {
      setEvents((events) => [{ type, payload, receivedAt: new Date().toISOString() }, ...events].slice(0, 25));
    };

    const onConnect = () => setConnected(true);
    const onDisconnect = () => setConnected(false);
    const onTenantCreated = (payload: any) => {
      setTenants((tenants) => [payload, ...tenants.filter((tenantItem) => tenantItem.id !== payload.id)]);
      pushEvent("tenant:created", payload);
    };
    const onTenantUpdated = (payload: any) => {
      setTenants((tenants) => tenants.map((tenantItem) => (tenantItem.id === payload.id ? payload : tenantItem)));
