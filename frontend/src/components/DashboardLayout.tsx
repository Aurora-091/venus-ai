...
export default function DashboardLayout({ children, tenant, tenants = [], onTenantChange }: Props) {
  ...
  const verticalIcon = { clinic: "🏥", hotel: "🏨", ecommerce: "🛍️" }[tenant?.vertical || ""] || "◈";
  const verticalColor = { clinic: "bg-emerald-500/10 text-emerald-400", hotel: "bg-blue-500/10 text-blue-400", ecommerce: "bg-violet-500/10 text-violet-400" }[tenant?.vertical || ""] || "bg-blue-500/10 text-blue-400";
      ...
      <aside className="w-[240px] shrink-0 flex flex-col border-r border-[#1E2A3E] bg-[#0A0E1A]">
        ...
        <div className="px-5 py-4 border-b border-[#1E2A3E]">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm tracking-tight">VoiceOS</span>
          </Link>
        </div>
          ...
          <div className="px-4 py-3 border-b border-[#1E2A3E]">
            <div className="flex items-center gap-2.5 bg-[#0F1623] rounded-xl px-3 py-2.5">
              ...
              <div className="min-w-0">
                <div className="text-xs font-medium text-white truncate">{tenant.name}</div>
                <div className={`text-[10px] px-1.5 py-0.5 rounded-full inline-block mt-0.5 ${verticalColor}`}>
                  {tenant.vertical}
                </div>
              </div>
            </div>
        ...
        {tenant && (
          <div className="px-4 py-3 border-t border-[#1E2A3E]">
            <div className="flex items-center gap-2 text-xs">
              <span className={`w-1.5 h-1.5 rounded-full ${hasRealAgent ? "bg-emerald-400 pulse-live" : "bg-[#475569]"}`} />
              <span className={hasRealAgent ? "text-emerald-400" : "text-[#475569]"}>
                {hasRealAgent ? "Agent connected" : "Not configured"}
              </span>
            </div>
            {hasRealPhoneNumber && tenant.phoneNumber && (
              <div className="font-mono text-[10px] text-[#475569] mt-1">{tenant.phoneNumber}</div>
            )}
          </div>
        )}
